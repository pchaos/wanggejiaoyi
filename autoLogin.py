...
def login(account, password, exe_path_filename, comm_password):
    assert os.path.isfile(exe_path_filename), "没有找到交易软件：{}".format(exe_path_filename)
    user = autologin.use(getclientname())
    user.prepare(
        user=account,
        password=password,
        exe_path=exe_path_filename,
